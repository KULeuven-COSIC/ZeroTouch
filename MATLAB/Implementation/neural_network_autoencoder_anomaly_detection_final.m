...
attacker_increase_sweep = [repelem(57.50, 105)' repelem(51.00, 105)' repelem(46.25, 105)' repelem(48.25, 105)' repelem(47.25, 105)' repelem(45.75, 105)' repelem(30.75, 105)' repelem(22.00, 105)' repelem(13.00, 105)' repelem(11.25, 105)' ...
                           repelem(15.50, 105)' repelem(14.00, 105)' repelem(14.25, 105)' repelem(8.50, 105)' repelem(15.50, 105)' repelem(15.25, 105)' repelem(15.00, 105)' repelem(13.00, 105)' repelem(10.25, 105)' repelem(10.00, 105)' ...
                           repelem(9.25, 105)' repelem(8.75, 105)' repelem(9.25, 105)' repelem(11.25, 105)' repelem(10.50, 105)' repelem(13.25, 105)' repelem(33.25, 105)' repelem(34.50, 105)' repelem(34.75, 105)' repelem(36.00, 105)' ...
                           repelem(37.50, 105)' repelem(34.25, 105)' repelem(34.00, 105)' repelem(27.25, 105)' repelem(18.75, 105)' repelem(11.50, 105)' repelem(8.75, 105)' repelem(22.75, 105)' repelem(10.50, 105)' repelem(5.5, 105)' ...
                           repelem(3.75, 105)' repelem(7.25, 105)' repelem(9.75, 105)' repelem(9.25, 105)' repelem(93.75, 105)' repelem(73.00, 105)' repelem(77.00, 105)' repelem(77.25, 105)' repelem(76.25, 105)' repelem(75.75, 105)' ...
                           repelem(62.00, 105)' repelem(66.00, 105)' repelem(100.00, 105)' repelem(88.00, 105)' repelem(66.25, 105)' repelem(67.75, 105)' repelem(65.25, 105)' repelem(67.50, 105)' repelem(52.50, 105)' repelem(38.75, 105)' ...
                           repelem(36.25, 105)' repelem(34.75, 105)' repelem(29.25, 105)' repelem(29.75, 105)' repelem(31.00, 105)' repelem(63.00, 105)' repelem(62.75, 105)' repelem(63.50, 105)' repelem(61.75, 105)' repelem(59.75, 105)' ...
                           repelem(55.75, 105)' repelem(42.00, 105)' repelem(43.75, 105)' repelem(49.75, 105)' repelem(43.75, 105)' repelem(46.75, 105)' repelem(45.5, 105)' repelem(21.50, 105)' repelem(18.00, 105)' repelem(13.00, 105)' ...
                           repelem(12.50, 105)' repelem(10.00, 105)' repelem(10.25, 105)' repelem(4.50, 105)' repelem(8.25, 105)' repelem(6.50, 105)' repelem(13.50, 105)' repelem(10.75, 105)' repelem(14.00, 105)' repelem(9.75, 105)' ...
                           repelem(16.25, 105)' repelem(14.25, 105)' repelem(14.50, 105)' repelem(14.25, 105)' repelem(15.75, 105)' repelem(14.25, 105)' repelem(12.50, 105)' repelem(10.50, 105)' repelem(12.75, 105)' repelem(11.50, 105)' ...
                           repelem(12.00, 105)' repelem(10.25, 105)']; % optimal power levels for every outside transmitter, based on the power sweep analysis
...
for i=1:length(outside_data_cropped)
    ...
    % outside_data_cropped{i} = outside_data{i}(:,6) + attacker_increase_sweep(:,i);
end
...
for i = 1:size(X_train, 2)
    ...
    for j = 1:num_augmentations
        ...
        % noisy_sample = original_sample; % add no noise
...
TNR = zeros(1, numThresholds);
FNR = zeros(1, numThresholds);
FP_array = [];
FN_array = [];
...
figure;
plot(thresholds, FPR, 'g', 'LineWidth', 2, 'DisplayName', 'False Positives');
hold on;
plot(thresholds, FNR, 'b', 'LineWidth', 2, 'DisplayName', 'False Negatives');
legend show;
xlabel('Threshold value');
ylabel('Error Rate');
title('False Positives and False Negatives curves for all samples')
hold off;

FPR_sweep = FPR;
FNR_sweep = FNR;
% thresholds_7dB = thresholds;

% save('thresholds.mat', 'thresholds');
% save('FPR_sweep.mat', 'FPR_sweep');
% save('FNR_sweep.mat', 'FNR_sweep');
...
TPR_majority = zeros(1, numThresholds);
FPR_majority = zeros(1, numThresholds);
TNR_majority = zeros(1, numThresholds);
FNR_majority = zeros(1, numThresholds);

for i = 1:numThresholds
    % Classify based on threshold
    anomaly_labels_temp_2 = reconErrorTest > thresholds(i);
    
    % Majority voting strategy confusion matrix without optimal thershld
    subarrays = reshape(anomaly_labels_temp_2, num_augmentations + 1, []).';
    anomaly_labels_o_temp = [];
    for j=1:length(subarrays(:,1))
         if sum(subarrays(j,:)) >= int16((num_augmentations + 1)/2)
            anomaly_labels_o_temp = [anomaly_labels_o_temp 1];
         else
            anomaly_labels_o_temp = [anomaly_labels_o_temp 0];
         end
    end

    % Compute confusion matrix components
    TP_majority = sum((anomaly_labels_o_temp == 1) & (true_labels_test_o == 1));
    TN_majority = sum((anomaly_labels_o_temp == 0) & (true_labels_test_o == 0));
    FP_majority = sum((anomaly_labels_o_temp == 1) & (true_labels_test_o == 0));
    FN_majority = sum((anomaly_labels_o_temp == 0) & (true_labels_test_o == 1));

    % Calculate TPR and FPR
    TPR_majority(i) = TP_majority / (TP_majority + FN_majority);
    FPR_majority(i) = FP_majority / (FP_majority + TN_majority);
    TNR_majority(i) = TN_majority / (TN_majority + FP_majority);
    FNR_majority(i) = FN_majority / (FN_majority + TP_majority);

end

figure;
plot(thresholds, FPR_majority, 'g', 'LineWidth', 2, 'DisplayName', 'False Positives');
hold on;
plot(thresholds, FNR_majority, 'b', 'LineWidth', 2, 'DisplayName', 'False Negatives');
legend show;
xlabel('Threshold value');
ylabel('Error Rate');
title('False Positives and False Negatives curves for majority voting')
hold off;

FPR_sweep_majority = FPR_majority;
FNR_sweep_majority = FNR_majority;

% save('FPR_sweep_majority.mat', 'FPR_sweep_majority');
% save('FNR_sweep_majority.mat', 'FNR_sweep_majority');
...
% title('Stem Plot of Average Errors of a Train set per sample (train)'); % (number of receivers) * (number of samples per receiver) = total namuber of samples
% xlabel('Sample Index');
...
% % Add legend
% hLegend = legend('Error per transmitter', 'Optimal Threshold', 'Min error in training', 'Extreme Value Theory (EVT)', 'Interquartile Range (IQR) Method', 'Max error in training', 'Median error in training', 'Mean error in training','Z-Score', 'Percentile', 'Parametric Statistical Modeling', 'Gaussian Mixture Model (GMM)', 'Location', 'best');
% title('Stem Plot of Average Errors of a Train set per receiver (train)');
...
% groups = repelem(1:187, 6)';
% figure
% boxplot(reconErrorTest', groups, 'PlotStyle', 'compact');
% yline(threshold_IQR, 'b', 'LineWidth', 1.4);
% xlabel('Transmitter Index');
% ylabel('Reconstruction Error');
% 
% save('recon_error_99.75.mat', 'reconErrorTest');
